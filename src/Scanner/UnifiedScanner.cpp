...
// Finds the matching closing brace, respecting strings and comments.
...
// This is the final, robust, single-pass implementation.
ScannedOutput UnifiedScanner::scan(const std::string& source) {
    ...
    for (size_t i = 0; i < source.length(); ++i) {
        ...
        // Update state to ignore keywords in strings/comments
        switch (state) {
            case ParseState::NORMAL:
                if (current == '"') state = ParseState::IN_DOUBLE_QUOTE_STRING;
                else if (current == '\'') state = ParseState::IN_SINGLE_QUOTE_STRING;
                else if (current == '`') state = ParseState::IN_TEMPLATE_LITERAL;
                else if (current == '/' && i + 1 < source.length()) {
                    if (source[i + 1] == '/') state = ParseState::IN_LINE_COMMENT;
                    else if (source[i + 1] == '*') state = ParseState::IN_BLOCK_COMMENT;
                }
                break;
            case ParseState::IN_DOUBLE_QUOTE_STRING: if (current == '"' && prev != '\\') state = ParseState::NORMAL; break;
            case ParseState::IN_SINGLE_QUOTE_STRING: if (current == '\'' && prev != '\\') state = ParseState::NORMAL; break;
        ...
        if (state != ParseState::NORMAL) continue;
        ...
        if (is_style || is_script || is_listen) {
            ...
            if (is_listen) {
                std::string block_content = source.substr(i, block_end - i + 1);
                std::string placeholder = "/*_CHTL_PLACEHOLDER_" + std::to_string(placeholder_id_counter++) + "_*/";
                this->output.fragments[placeholder] = {FragmentType::CHTL_JS, block_content};
                ...
                if (it_begin == it_end) {
                    processed_source.append(source, i, block_end - i + 1);
                } else {
...
// This function is conceptually replaced by the new scan implementation.
