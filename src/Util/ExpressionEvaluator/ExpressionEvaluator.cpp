    ...
    std::string evaluate_recursive(const ExpressionNode* node, const BaseNode* astRoot, const BaseNode* contextNode, std::vector<const ExpressionNode*>& call_stack) {
        ...
        switch (node->getType()) {
            ...
            case NodeType::BinaryOp: {
                ...
                if (opNode->getOp() == TokenType::LOGICAL_AND || opNode->getOp() == TokenType::LOGICAL_OR) {
                    std::string left_str = evaluate_recursive(opNode->getLeft(), astRoot, contextNode, call_stack);
                    std::string right_str = evaluate_recursive(opNode->getRight(), astRoot, contextNode, call_stack);
                    bool left_bool = (left_str == "true" || (left_str != "false" && !left_str.empty() && stod(left_str) != 0));
                    bool right_bool = (right_str == "true" || (right_str != "false" && !right_str.empty() && stod(right_str) != 0));
                    if (opNode->getOp() == TokenType::LOGICAL_AND) {
                        result_str = (left_bool && right_bool) ? "true" : "false";
                    } else {
                        result_str = (left_bool || right_bool) ? "true" : "false";
                    }
                    break;
                ...
                switch (opNode->getOp()) {
                    ...
                    case TokenType::GREATER: result_str = (left.value > right.value) ? "true" : "false"; break;
                    case TokenType::LESS: result_str = (left.value < right.value) ? "true" : "false"; break;
                    case TokenType::GREATER_EQUAL: result_str = (left.value >= right.value) ? "true" : "false"; break;
                    case TokenType::LESS_EQUAL: result_str = (left.value <= right.value) ? "true" : "false"; break;
                    case TokenType::EQUAL_EQUAL: result_str = (left.value == right.value) ? "true" : "false"; break;
                    case TokenType::NOT_EQUAL: result_str = (left.value != right.value) ? "true" : "false"; break;
            ...
            case NodeType::Reference: {
                 ...
                 if (selector == "self") {
                     if (!contextNode) throw std::runtime_error("Reference error: 'self' used without a context.");
                 ...
                 if (!propertyExpr) {
                     auto attributes = targetNode->getAttributes();
                     if (attributes.count(propertyName)) {
                         result_str = attributes.at(propertyName);
                     } else {
                        throw std::runtime_error("Reference error: property '" + propertyName + "' not found on element with selector '" + selector + "'.");
                     }
                 } else {
