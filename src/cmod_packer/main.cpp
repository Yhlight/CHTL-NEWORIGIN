...
// Generates the [Export] block string from a template manager.
std::string generate_export_block(const TemplateManager& manager) {
    std::stringstream ss;
    ss << "\n\n// Auto-generated by cmod_packer\n";
    ss << "[Export]\n{\n";

    auto all_templates = manager.getAllTemplates();
    for (const auto& pair : all_templates) {
        // pair.first is namespace, pair.second is map of name->template
        for (const auto& tmpl_pair : pair.second) {
            ...
            if (dynamic_cast<const StyleTemplateNode*>(tmpl_node)) {
                type_qualifier = "@Style";
            } else if (dynamic_cast<const ElementTemplateNode*>(tmpl_node)) {
                type_qualifier = "@Element";
            } else if (dynamic_cast<const VarTemplateNode*>(tmpl_node)) {
                type_qualifier = "@Var";
            }

            if (!type_qualifier.empty()) {
...
int main(int argc, char* argv[]) {
    ...
    if (!fs::is_regular_file(info_file_path)) {
        std::cerr << "Error: Main info file '" << info_file_path << "' not found." << std::endl;
        return 1;
    }

    // 1. Parse all source files to find exports
    TemplateManager manager;
    parse_module_sources(src_subdir, manager);

    // 2. Generate the [Export] block
    ...
    try {
        ...
        // Add the src directory as before
        add_directory_to_archive(zf, src_subdir, source_dir);

        // Add the MODIFIED info file content to the archive
