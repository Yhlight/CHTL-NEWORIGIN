...
TEST_CASE(basic_element_tokenization)
    ...
    ASSERT_EQ(CHTL::TokenType::IDENTIFIER, tokens[0].type);
END_TEST_CASE
...
TEST_CASE(string_literals)
    ...
    ASSERT_EQ(CHTL::TokenType::STRING_LITERAL, tokens[0].type);
END_TEST_CASE
...
TEST_CASE(numbers)
    ...
    ASSERT_EQ(CHTL::TokenType::NUMBER_LITERAL, tokens[0].type);
